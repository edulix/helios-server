...
function verify_ballot(election_raw_json, encrypted_vote_json, status_cb) {
	...
	status_cb("{% trans "election fingerprint is " %}" + election_hash);
	
	// display ballot fingerprint
	encrypted_vote = HELIOS.EncryptedVote.fromJSONObject(encrypted_vote_json, election);
	status_cb("{% trans "smart ballot tracker is " %}" + encrypted_vote.get_hash());
	
      // check the hash
      if (election_hash == encrypted_vote.election_hash) {
          status_cb("{% trans "election fingerprint matches ballot" %}");
      } else {
          overall_result = false;
          status_cb("{% trans "PROBLEM = election fingerprint does not match" %}");
      }
      
      // display the ballot as it is claimed to be
      status_cb("{% trans "Ballot Contents:" %}");
      _(election.questions).each(function(q, qnum) {
	      if (q.tally_type != "homomorphic") {
		  status_cb("{% trans "WARNING: the tally type for this question is not homomorphic. Verification may fail because this verifier is only set up to handle homomorphic ballots." %}");
	      }
        
	      var answer_pretty_list = _(encrypted_vote.encrypted_answers[qnum].answer).map(function(aindex, anum) {
		      return q.answers[aindex];
		  });
	      status_cb("{% trans "Question #" %}" + (qnum+1) + " - " + q.short_name + " : " + answer_pretty_list.join(", "));
      });
      
      // verify the encryption
      if (encrypted_vote.verifyEncryption(election.questions, election.public_key)) {
          status_cb("{% trans "Encryption Verified" %}");
      } else {
          overall_result = false;
          status_cb("{% trans "PROBLEM = Encryption doesn't match." %}");
      }
      
      // verify the proofs
      if (encrypted_vote.verifyProofs(election.public_key, function(ea_num, choice_num, result) {
      })) {
          status_cb("{% trans "Proofs ok." %}");
      } else {
          overall_result = false;
          status_cb("{% trans "PROBLEM = Proofs don't work." %}");
      }
    } catch (e) {
      status_cb('{% trans "problem parsing election or ballot data structures, malformed inputs" %}: ' + e.toString());
      overall_result=false;
    }
